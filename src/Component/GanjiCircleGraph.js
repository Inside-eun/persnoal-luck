  ...
  useEffect(() => {
    ...
    if (
      ...
      const updatePoints = (element) => {
        ...
        if (earthSource.includes(element)) updatedPoints.金 += 1;
        if (metalSource.includes(element)) updatedPoints.金 += 1;
        if (woodSource.includes(element)) updatedPoints.木 += 1;

        const maxKey = Object.keys(updatedPoints).reduce((a, b) =>
          updatedPoints[a] >= updatedPoints[b] ? a : b
        );

        const minKey = Object.keys(updatedPoints).reduce((a, b) =>
          updatedPoints[a] < updatedPoints[b] ? a : b
        );
    ...
    <>
      ...
      {pointReady ? (
        <div style={{ width: "100%", height: "320px" }}>
          ...
          >
            <h3 style={{ margin: 0, fontSize: "32px", lineHeight: "1.2" }}>
              {pointStore[0]}
            </h3>
          </div>
        </div>
      ...
      <style.PointScript>
        ...
        <br />
        부족한 기운을 채울 수 있는 녹색 물건들을 보여드릴게요
      </style.PointScript>
      <style.DivideLine />
    </>
  );
